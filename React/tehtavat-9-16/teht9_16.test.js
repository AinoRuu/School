...
describe("Tehtävä 9 Cars-komponentti (1p)", () => {
  test("Tehtävä 9, Cars-komponentti (1p)", () => {
    render(<Cars />);
    for (let i = 0; i < 6; i++) {
...
describe("Tehtävä 10 Info-komponentti (1p)", () => {
  test("Tehtävä 10, Info-komponentti (0.5p)", () => {
    render(<Info count={6}></Info>);
    let h2 = screen.getByRole("heading");
    expect(h2).toHaveTextContent("Ainakin 5 nimeä on jo syötetty");
  });

  test("Tehtävä 10, lisätään väh. 5 autoa (0.5p)", () => {
    render(<Cars />);
    for (let i = 0; i < 6; i++) {
      userEvent.type(screen.getByRole("textbox"), "Volvo");
      const save = screen.getByText(/Save/i);
...
describe("Tehtävä 11, ListForm-komponentti (1p)", () => {
  test("Tehtävä 11, ListForm-komponentti (1p)", () => {
...
describe("Tehtävä 12, Error-komponentti (1p)", () => {
  test("Tehtävä 12, virheviestit (0.5p)", () => {
    const { debug } = render(<ListForm />);
    ...
    expect(errortext).toHaveTextContent(
      "Virheelliset kentät: nimi,osoite,vuosi"
    );
  ...
  test("Tehtävä 12, Error-komponentti, virheelliset syötteet (0.25p)", () => {
  ...
  test("Tehtävä 12, Error-komponentti, oikeat syötteet (0.25p)", () => {
...
describe("Tehtävä 13, tarkistetaan onko nimi jo syötetty (1p)", () => {
  test("Tehtävä 13, virheviestit (0.5p)", () => {
    const { debug } = render(<ListForm />);
  ...
  test("Tehtävä 13, ErrorMessage-komponentti, virheelliset syötteet (0.5p)", () => {
...
const testTable = (nimi, ammatti, tutkinto_suoritettu, tutkinto) => {
  ...
  if (tutkinto_suoritettu) expect(td[2]).toHaveTextContent(tutkinto_suoritettu);
  if (tutkinto) expect(td[3]).toHaveTextContent(tutkinto);
};

describe("Tehtävä 14, Professional komponentti (1p)", () => {
  test("Tehtävä 14, syötetään nimi ja ammatti (1p)", () => {
...
describe("Tehtävä 15, Professional komponentti (1p)", () => {
  test("Tehtävä 15, syötetään tutkinto suoritettu (1p)", () => {
    const { debug } = render(<Professional ammatit={ammatit} />);
    let nimiInput = screen.getByLabelText(/Nimi/i);
    let ammattiSelect = screen.getByLabelText(/Ammatti/i);
    const insert = screen.getByText(/Insert/i);
...
describe("Tehtävä 16, Professional komponentti (1p)", () => {
  test("Tehtävä 16, syötetään tutkinto (1p)", () => {
    const { debug } = render(<Professional ammatit={ammatit} />);
    let nimiInput = screen.getByLabelText(/Nimi/i);
    let ammattiSelect = screen.getByLabelText(/Ammatti/i);
    const insert = screen.getByText(/Insert/i);
